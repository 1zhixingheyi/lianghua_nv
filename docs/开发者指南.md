...
MYSQL_DATABASE=lianghua_dev

CLICKHOUSE_HOST=localhost
CLICKHOUSE_PORT=9000
CLICKHOUSE_DATABASE=lianghua_ch_dev
...
**存储层配置 (config/schemas/)**:
- `mysql.yaml`: MySQL结构化数据存储配置
- `clickhouse.yaml`: ClickHouse分析数据存储配置
- `redis.yaml`: Redis缓存数据存储配置
- `minio.yaml`: MinIO对象数据存储配置

**业务模块配置 (config/modules/)**:
- `trading.yaml`: 交易策略和风控配置
- `data_integrity.yaml`: 数据质量和完整性配置
...
redis_host = redis_config['connection']['host']

# 获取交易配置
trading_config = get_trading_config()
max_position = trading_config['risk_management']['max_position_ratio']
```
